...
def extract_decompilation(decompilation, window=320):
    '''
    Reduces the size of the function's decompilation so it'll fit in the "ExplainFurther" query.
    The resulting extracted decompilation will contain all the comments containing format strings.
    ...
    end_match = re.search(r'(\}[0-9A-Za-z_]*\{)', decompilation[::-1])
    start = start_match.start() if start_match is not None else 0
    end = len(decompilation) - (end_match.end() if end_match is not None else 0)
